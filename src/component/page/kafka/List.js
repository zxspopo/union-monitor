...
import { listKafka, removeKakfa } from './api';
...
class List extends React.Component {
  ...
  onDetail = (id) => {
    this.props.history.push("/page/kafka/detail/" + id)
  }
  ...
  onRemove = (id) => {
    ...
    confirm({
      ...
      // content: '确定删除这条记录?',
      onOk() {
        removeKakfa({ "id": id }, function (data) {
          message.info("删除成功!");
          self.onSearch();
        })
      },
  ...
  onAdd = () => {
    this.props.history.push("/page/kafka/addKafka")
  }
  render() {
    const columns = [
      {
